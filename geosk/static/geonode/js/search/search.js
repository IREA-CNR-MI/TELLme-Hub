...
(function(){
  ...
  module.load_keywords = function ($http, $rootScope, $location){
        ...
        $http.get(KEYWORDS_ENDPOINT, {params: params}).success(function(data){
            if($location.search().hasOwnProperty('keywords__slug__in')){ //TODO: check if it is necessary to change in keywords__slug or not
                data.objects = module.set_initial_filters_from_query(data.objects,
                    $location.search()['keywords__slug__in'], 'slug'); //TODO: check if it is necessary to change in keywords__slug or not
            }
